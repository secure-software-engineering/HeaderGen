...
response_file  = "/tmp/evaluation/user_study/Responses.csv"
out_path = "/tmp/evaluation/user_study/individual_responses"
...
def write_results(data, results_path):
    header = ["Project", "Precision_1", "Recall_1", "Precision_2", "Recall_2", "Studyid"]
    prec_sum_1 = 0
    rec_sum_1 = 0
    prec_sum_2 = 0
    rec_sum_2 = 0
    ...
    with open(results_path, "w+") as f:
        ...
        for project, dt in data.items():
            writer.writerow([project, dt["precision_1"], dt["recall_1"], dt["precision_2"], dt["recall_2"], dt["study_id"]])
            try:
                ...
                continue
        writer.writerow(["Average", round(prec_sum_1/cnt,1),
            round(rec_sum_1/cnt,1), round(prec_sum_2/cnt,1),
            round(rec_sum_2/cnt,1)])
        print("P:", round(prec_sum_1/cnt,1), "P:", round(prec_sum_2/cnt,1), "R:", round(rec_sum_1/cnt,1),  "R:", round(rec_sum_2/cnt,1))
...
for i in range(2, 10):
    ...
    questions_data[i] = {
        k:v for k, v in precision_questions.items()
    }

    questions_data[i] = questions_data[i] | {k:v for k, v in recall_questions.items()} | {"study_id": actual['study_id']}
