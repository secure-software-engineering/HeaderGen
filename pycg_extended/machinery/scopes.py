...
class ScopeManager(object):
    ...
    def handle_module(self, modulename, filename, contents):
        ...
        def process(namespace, parent, table):
            ...
            if name:
                # NOTE: Flow Sensitive
                fullns = utils.join_ns(
                    namespace, "{}:{}".format(name, table.get_lineno())
                )
            else:
                fullns = namespace
...
class ScopeItem(object):
    ...
    def get_obj_counter(self, name):
        if name in self.obj_counter:
            return self.obj_counter[name]

    def inc_obj_counter(self, name, lineno, val=1):
        if name not in self.obj_counter:
            self.obj_counter[name] = {
                "count": 0,
                "line_ref": {},
            }

        self.obj_counter[name]["count"] += val
        self.obj_counter[name]["line_ref"][lineno] = self.obj_counter[name]["count"]
        return self.obj_counter[name]
