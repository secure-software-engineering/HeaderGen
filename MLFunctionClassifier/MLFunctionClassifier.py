...
class MLFunctionClassifier:
    ...
    def __init__(self):
        ...
        self.fe_labels = ['Feature Engineering','Feature Selection','Feature Transformation']

        self.mb_labels = ['Model Building and Training','Model Parameter Tuning','Model Training','Model Validation and Assembling']

    def predict_dp(self, preprocessed_docstring):
        dp_numerical_vector = self.dp_vectorizer.transform([preprocessed_docstring])
        dp_predicted_probabilities = self.dp_classifier.predict_proba(dp_numerical_vector)
    ...
    def predict_fe(self, preprocessed_docstring):
        ...
        fe_predicted_probabilities = self.fe_classifier.predict_proba(fe_numerical_vector)
    ...
    def predict_mb(self, preprocessed_docstring):
        ...
        mb_predicted_probabilities = self.mb_classifier.predict_proba(mb_numerical_vector)
    ...
    def predict_function(self, docstring):
        
        preprocessed_docstring = data_cleaning(docstring)
        if len(preprocessed_docstring.split()) < 10:
            return []
        
        highlevel_numerical_vector = self.highlevel_vectorizer.transform([preprocessed_docstring])
        highlevel_predicted_probabilities = self.highlevel_classifier.predict_proba(highlevel_numerical_vector)
        highlevel_predicted_label = (highlevel_predicted_probabilities > 0.5).astype(int)
